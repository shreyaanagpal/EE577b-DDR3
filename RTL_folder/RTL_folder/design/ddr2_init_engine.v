...
begin
	...
	// ----------------------------------------------------------
        ...
        // Rest of the values are set to desired values too (else it throws errors)
			...
			// FILL IN VALUES
                	a[5:3] <= 011; // Additive CAS
                	a[9:7] <= 111; // OCD Calibration Program
			//
        ...
        // Issue precharge all command.
        	...
        	`S_PRE2: begin
                ...
                `S_NOP7: {csbar, rasbar, casbar, webar} <= 0111; // NOP command
        ...
        // bring the clock enable (cke) down
        	...
        	`S_AUTO1: begin
                	{csbar, rasbar, casbar, webar} <= 0001;
					cke <= 0;				// Auto Refresh Command 
	                end
                `S_NOP8: {csbar, rasbar, casbar, webar} <= 0111; // NOP command
        ...
        // Exit self refresh to a read command (tXSRD) =  200 cycles = 800 ns
	        ...
	        `S_AUTO2: begin
        	        {csbar, rasbar, casbar, webar} <= 0001; // Auto Refresh Command
	                end
                `S_NOP9: {csbar, rasbar, casbar, webar} <= 0111; // NOP command
	...
	// Need to wait same time as last refresh before issuing the next command
        ...
        // (i.e. to program operating parameters without resetting the DLL.
			...
			a[7] <= 0; // TM = Normal
                ...
                `S_NOP10: {csbar, rasbar, casbar, webar} <= 0111 ; // NOP command
        ...
        // ----------------------------------------------------------
			...
			a[1] <= 0; // Output Driver Impedence
        	        a[2] <= 1; // Rtt ODT
        	        a[6] <= 0; // Rtt ODT
        	        a[5:3] <= 010 ; // Additive CAS Latency
			a[9:7] <= 3'b111; // OCD Calibration
                ...
                `S_NOP11: {csbar, rasbar, casbar, webar} <= 0111; // NOP command
	...
	// OCD Must be at least 800 ns away from issuing the DLL Reset command
			...
			{csbar, rasbar, casbar, webar} <= 0000; // EMRS Command
                ...
                `S_NOP12: {csbar, rasbar, casbar, webar} <= 0111; // NOP command
	...
	// ----------------------------------------------------------
        ...
        // ----------------------------------------------------------
	        ...
	        `S_PRE3: begin
                ...
                `S_NOP13: {csbar, rasbar, casbar, webar} <= 0111; // NOP command
